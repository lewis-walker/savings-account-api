...
**Idempotency orchestration sits beside the controller, not in a filter.** `IdempotentAccountOpening` owns the claim, replay, complete and release; the controller calls it once. A filter was considered and rejected for three reasons: it would have to buffer and replay the response body, where this stores the account id and rebuilds the answer; it sees raw bytes, so the request fingerprint would cover whitespace and reformatting would defeat a retry; and it would need ordering after authentication to scope keys by customer, adding a second ordering constraint to a filter chain that already has one. It is deliberately concrete rather than a generic template, because there is one idempotent endpoint — a second is where the mechanism gets generalised, from two examples rather than a guess.
