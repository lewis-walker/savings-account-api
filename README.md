...
| Nickname (optional) | `OpenAccountRequest`. An account left unnamed is given a `displayName` from the customer's own account sequence, so five accounts are told apart; `nickname` stays null | `AccountApiTest` |
